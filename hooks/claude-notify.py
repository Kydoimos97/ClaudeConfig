...
def get_icon_path() -> str:
    ...
    if sys.platform == "win32":
        icon_name = f"{PRESETS[preset]['icon']}.ico"
    else:
        icon_name = f"{PRESETS[preset]['icon']}.png"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: claude-notify.py <preset> [detail] [error_snippet]", file=sys.stderr)
        sys.exit(1)
    preset = sys.argv[1]
    if preset not in PRESETS:
        print(f"Unknown preset: {preset} (use: approval, completed, elicitation, notification, error)", file=sys.stderr)
        sys.exit(1)
